...
export const locationHover = () => {
    ...
    const mediaGueryXL = window.matchMedia('(min-width: 1240px)');
    const mediaGueryLG = window.matchMedia('(min-width: 1024px)');

    for (const item of locationItems) {
        const content = item.querySelector('.location__content');
        const title = item.querySelector('.location__title');
        const description = item.querySelector('.location__dscription');
        ...
        mediaGueryLG.addEventListener('change', (e) => {
            if (!e.matches) {
                content.style = '';
                title.style = '';
                description.style = '';
